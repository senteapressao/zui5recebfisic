sap.ui.define([
    ...
    return BaseController.extend("zui5recebfisic.controller.AssembleUCLxC", {
        ...
        onInit: function () {
            ...
            this.getRouter().getRoute("RouteAssembleUCLxC").attachPatternMatched(this._onRouteMatched, this);
        },
        ...
        _onRouteMatched: function (oEvent) {
            ...
            setTimeout(function () {
                ...
                // Ensure inputs are reset/enabled for a new UC session before loading items
                this._enableMaterialInput();
                this._clearInputs();
                // Always update scanned items list when navigating to assembleUCLxC page
        ...
        _setFocusToMaterialInput: function () {
            ...
            delays.forEach(function (delay) {
                setTimeout(function () {
                    var oMaterialInput = that.byId("assembleUCLxCMaterialInput");
        ...
        _focusQuantityField: function () {
            ...
            delays.forEach(function (delay) {
                setTimeout(function () {
                    var oQuantityInput = that.byId("assembleUCLxCQuantidadeInput");
        ...
        _populateDisplayFields: function (oReceiptData) {
            ...
            if (oReceiptData.nf) {
                this.byId("assembleUCLxCNfDisplay").setValue(oReceiptData.nf);
            }

            if (oReceiptData.identificador) {
                this.byId("assembleUCLxCIdentificadorDisplay").setValue(oReceiptData.identificador);
            }

            if (oReceiptData.uc) {
                this.byId("assembleUCLxCUcDisplay").setValue(oReceiptData.uc);
            }
         ...
         * This method ensures the list is always refreshed when navigating to assembleUCLxC
         * @private
         */
        _updateScannedItemsOnPageLoad: function () {
            ...
            setTimeout(function () {
                ...
                if (oReceiptContext && oReceiptContext.nf && oReceiptContext.identificador && oReceiptContext.uc) {
                    ...
                    // Enable material input if there are no items for the (new) UC yet
                    that._enableMaterialInput();
                    ...
                    oModel.setProperty("/scannedItems", []);
                    that._updateScannedItemsCount();
                    // Also ensure input is enabled in this case
                    that._enableMaterialInput();
                }
            }, 200);
        },
            ...
            this.getItemsForUC(
                ...
                if (oItemsResult && oItemsResult.items && oItemsResult.items.length > 0) {
                    ...
                    if (aDisplayItems.length > 0) {
                        ...
                        // LxC MODE: Disable material input if items are loaded (only 1 material per UC)
                        that._disableMaterialInput();
                    }
        ...
        _populateYearSelect: function () {
            var oYearSelect = this.byId("assembleUCLxCAnoSelect");
        ...
        onMaterialInputSubmit: function (oEvent) {
            ...
            // LxC MODE: Check if there's already an item (only 1 material per UC)
            var oModel = this.getView().getModel();
            var aScannedItems = oModel.getProperty("/scannedItems") || [];
            if (aScannedItems.length > 0) {
                this.showErrorMessage("UC LxC permite apenas 1 material por UC. Remova o item existente antes de adicionar outro.");
            ...
            // In LxC mode, we ONLY validate and fill quantity - we don't auto-add the item
            this._validateBarcodeWithABAPForSubmit(sValue, oInput)
                .then(function (oMaterialData) {
                    ...
                    // Show Lastro and Camada fields after material validation
                    that.byId("assembleUCLxCLastroContainer").setVisible(true);
                    that.byId("assembleUCLxCCamadaContainer").setVisible(true);
                    
                    // Focus on Lastro field for user input
                    setTimeout(function() {
                        var oLastroInput = that.byId("assembleUCLxCLastroInput");
                        if (oLastroInput && oLastroInput.getDomRef()) {
                            oLastroInput.focus();
                        }
                    }, 300);
                    
                    // Show info message to guide user
                    //that.showInfoMessage("Material validado. Preencha Lastro e Camada e clique em 'Adicionar Item'.");
                })
                .catch(function () {
        ...
        onMaterialInputChange: function (oEvent) {
            ...
            if (sValue.length === 0) {
                this.byId("assembleUCLxCValidadeContainer").setVisible(false);
                return;
            }
        ...
        _validateBarcodeWithABAP: function (sBarcode, oInput) {
            ...
            // Clear previous validation state
            this.clearInputValidation("assembleUCLxCMaterialInput");
            ...
            if (this._recebEmb === "X") {
                ...
                if (!oValidation.isValid) {
                    this.setInputError("assembleUCLxCMaterialInput", oValidation.message);
                    this.byId("assembleUCLxCValidadeContainer").setVisible(false);
            ...
            this._performMaterialLookup(oValidation.cleanBarcode)
                .then(function (oMaterialData) {
                    ...
                    that.setInputSuccess("assembleUCLxCMaterialInput", "Material válido: " + oMaterialData.materialDescription);
                    ...
                    if (that._recebEmb === "X") {
                        ...
                        that.byId("assembleUCLxCQuantidadeContainer").setVisible(true);
                        // Clear any previously auto-filled value
                        that.byId("assembleUCLxCQuantidadeInput").setValue("");
                    } else {
                        ...
                        if (oMaterialData.quantity && oMaterialData.quantity !== "" && !isNaN(oMaterialData.quantity)) {
                            that.byId("assembleUCLxCQuantidadeInput").setValue(oMaterialData.quantity);
                        }
                    }

                    // Auto-fill Material SKU field with MaterialSKU value from backend
                    if (oMaterialData.materialSKU && oMaterialData.materialSKU !== "") {
                        that.byId("assembleUCLxCMaterialSKUContainer").setVisible(true);
                        that.byId("assembleUCLxCMaterialSKUInput").setValue(oMaterialData.materialSKU);
                    }
        ...
        _canProceedWithAddItem: function (oMaterialData) {
            ...
            var sMes = this.byId("assembleUCLxCMesSelect").getSelectedKey();
            var sAno = this.byId("assembleUCLxCAnoSelect").getSelectedKey();
        ...
        _handleBarcodeValidationError: function (oError, sBarcode) {
            ...
            this.setInputError("assembleUCLxCMaterialInput", sErrorMessage);
            this.byId("assembleUCLxCValidadeContainer").setVisible(false);
        ...
        _addBarcodeInputShake: function () {
            var oInput = this.byId("assembleUCLxCMaterialInput");
            if (oInput) {
        ...
        _checkMaterialExpirationRequirement: function (oMaterialData) {
            var oValidadeContainer = this.byId("assembleUCLxCValidadeContainer");
            ...
            if (oMaterialData && oMaterialData.requiresExpiration) {
                ...
                if (this._firstExpirationDate) {
                    ...
                    this.byId("assembleUCLxCMesSelect").setSelectedKey(this._firstExpirationDate.mes);
                    this.byId("assembleUCLxCAnoSelect").setSelectedKey(this._firstExpirationDate.ano);
        ...
        onAdicionarItemPress: function () {
            var that = this;
            var sMaterial = this.byId("assembleUCLxCMaterialInput").getValue();
            var sQuantidadeBase = this.byId("assembleUCLxCQuantidadeInput").getValue();
            var sLastro = this.byId("assembleUCLxCLastroInput").getValue();
            var sCamada = this.byId("assembleUCLxCCamadaInput").getValue();
            var sMes = this.byId("assembleUCLxCMesSelect").getSelectedKey();
            var sAno = this.byId("assembleUCLxCAnoSelect").getSelectedKey();

            // Check if material is already validated
            ...
            // LxC MODE: Check if there's already an item (only 1 item allowed per UC)
            var oModel = this.getView().getModel();
            var aScannedItems = oModel.getProperty("/scannedItems") || [];
            if (aScannedItems.length > 0) {
                this.showErrorMessage("UC LxC permite apenas 1 material por UC. Remova o item existente antes de adicionar outro.");
            ...
            // Validate Lastro and Camada (required for LxC mode)
            if (!sLastro || sLastro.trim().length === 0 || isNaN(parseFloat(sLastro)) || parseFloat(sLastro) <= 0) {
                this.showErrorMessage("Lastro é obrigatório e deve ser um número maior que zero");
                this.setInputError("assembleUCLxCLastroInput", "Lastro é obrigatório");
                return;
            }

            if (!sCamada || sCamada.trim().length === 0 || isNaN(parseFloat(sCamada)) || parseFloat(sCamada) <= 0) {
                this.showErrorMessage("Camada é obrigatória e deve ser um número maior que zero");
                this.setInputError("assembleUCLxCCamadaInput", "Camada é obrigatória");
                return;
            }

            // Validate base quantity
            if (!sQuantidadeBase || sQuantidadeBase.trim().length === 0 || isNaN(parseFloat(sQuantidadeBase)) || parseFloat(sQuantidadeBase) <= 0) {
                this.showErrorMessage("Quantidade é obrigatória e deve ser um número maior que zero");
                return;
            }

            // Calculate final quantity: lastro x camada x quantidade
            var fLastro = parseFloat(sLastro);
            var fCamada = parseFloat(sCamada);
            var fQuantidadeBase = parseFloat(sQuantidadeBase);
            var fQuantidadeFinal = fLastro * fCamada * fQuantidadeBase;

            // Validate inputs (material, etc)
            if (!this._validateItemInputs(sMaterial, fQuantidadeFinal.toString())) {
                return;
            }
            ...
            // Add item to scanned items list with calculated quantity
            this._addScannedItem(sMaterial, fQuantidadeFinal.toString(), sDataValidade);
        ...
        _addScannedItem: function (sMaterial, sQuantidade, sDataValidade) {
            ...
            this._createItemsTableRecord(oNewItem)
                .then(function (oData) {
                    ...
                    // LxC MODE: Disable material input after adding item (only 1 material per UC)
                    that._disableMaterialInput();
                })
                .catch(function (oError) {
        ...
        _getReceiptContext: function () {
            ...
            var oDisplayContext = {
                nf: this.byId("assembleUCLxCNfDisplay").getValue() || "",
                identificador: this.byId("assembleUCLxCIdentificadorDisplay").getValue() || "",
                uc: this.byId("assembleUCLxCUcDisplay").getValue() || ""
            };
        ...
        _clearInputs: function () {
            this.byId("assembleUCLxCMaterialInput").setValue("");
            this.byId("assembleUCLxCQuantidadeInput").setValue("");
            this.byId("assembleUCLxCLastroInput").setValue("");
            this.byId("assembleUCLxCCamadaInput").setValue("");
            this.byId("assembleUCLxCMesSelect").setSelectedKey("");
            this.byId("assembleUCLxCAnoSelect").setSelectedKey("");
            this.byId("assembleUCLxCValidadeContainer").setVisible(false);
            this.byId("assembleUCLxCQuantidadeContainer").setVisible(false);
            this.byId("assembleUCLxCMaterialSKUContainer").setVisible(false);
            this.byId("assembleUCLxCLastroContainer").setVisible(false);
            this.byId("assembleUCLxCCamadaContainer").setVisible(false);

            // Clear material validation state
            this.byId("assembleUCLxCMaterialInput").setValueState("None");
            this.byId("assembleUCLxCMaterialInput").setValueStateText("");

            // Clear current material data
            this._currentMaterialData = null;

            // LxC MODE: Check if there's already an item - don't focus if item exists
            var oModel = this.getView().getModel();
            var aScannedItems = oModel.getProperty("/scannedItems") || [];
            if (aScannedItems.length === 0) {
                // Focus back to material input for next scan only if no items exist
                this.byId("assembleUCLxCMaterialInput").focus();
            }
        },

        /**
         * Disable material input fields after adding item (LxC mode - only 1 material per UC)
         * @private
         */
        _disableMaterialInput: function () {
            this.byId("assembleUCLxCMaterialInput").setEnabled(false);
            this.byId("assembleUCLxCMaterialInput").setValue("");
            this.byId("assembleUCLxCMaterialInput").setPlaceholder("Limite de 1 material por UC atingido");
            this.byId("assembleUCLxCQuantidadeContainer").setVisible(false);
            this.byId("assembleUCLxCMaterialSKUContainer").setVisible(false);
            this.byId("assembleUCLxCLastroContainer").setVisible(false);
            this.byId("assembleUCLxCCamadaContainer").setVisible(false);
        ...
        /**
         * Enable material input fields when item is removed (LxC mode)
         * @private
         */
        _enableMaterialInput: function () {
            this.byId("assembleUCLxCMaterialInput").setEnabled(true);
            this.byId("assembleUCLxCMaterialInput").setPlaceholder("Escaneie ou digite o código de barras");
            this.byId("assembleUCLxCMaterialInput").setValue("");
        ...
        _showItemActionSheet: function (iItemIndex) {
            ...
            this._oItemActionSheet.openBy(this.byId("assembleUCLxCScannedItemsList").getItems()[iItemIndex]);
        },
        ...
        _performItemRemoval: function (iItemIndex, oItemToRemove) {
            ...
            this._deleteItemsTableRecord(oItemToRemove)
                .then(function () {
                    ...
                    // If no items left, reset Material SKU validation and re-enable material input
                    if (aScannedItems.length === 0) {
                        that._resetMaterialSKUValidation();
                        // LxC MODE: Re-enable material input when all items are removed
                        that._enableMaterialInput();
                    }
        ...
        _saveToGlobalContextForOccurrence: function (oReceiptData) {
            ...
            oGlobalData.uc = oReceiptData.uc; // UC is specific to assembleUCLxC context
            oGlobalData.currentStep = "Ocorrencia";
            oGlobalData.sourceScreen = "assembleUCLxC"; // Track source screen
